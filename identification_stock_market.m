...
for k=1:100
    ...
    a=dpurelin(net1'*w11-wo,yn(k))*ei(k); %gradient of o/p. its a scalar
    b=dtansig(x1(k)*w1+x2(k)*w2+x3(k)*w3+x4(k)*w4+x5(k)*w5+x6(k)*w6+x7(k)*w7+x8(k)*w8+x9(k)*w9+x10(k)*w10-wh,net1); %derivative of hidden neuron AF, vector
